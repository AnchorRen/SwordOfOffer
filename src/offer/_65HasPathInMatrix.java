 ...
 * 思路：
 * 
 * 		这是一个可以用回朔法解决的典型题。首先，在矩阵中任选一个格子作为路径的起点。假设矩阵中某个格子的字符为ch，
 * 那么这个格子不可能处在路径上的第i个位置。如果路径上的第i个字符不是ch，那么这个格子不可能处在路径上的第i个位置。
 ...
 * 		
 * @author REN
 * @date 2016年9月10日 下午2:55:15
 */
public class _65HasPathInMatrix {
    ...
    private static boolean hasPathCore(char[] matrix, int rows, int cols, char[] str, boolean[] visited,
        ...
        if (pathLength[0] == str.length) { //递归停止条件
            return true;
        }
